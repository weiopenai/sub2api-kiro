...
### 2026-06-21
- 新增 **Claude Opus 4.8** 支援：`claude-opus-4-8` 與 `claude-opus-4-8-thinking` 已加入 Kiro 模型清單，並對映到上游模型 `claude-opus-4.8`。
- 部署手冊（[deploy/usage-guide-zh-TW.html](deploy/usage-guide-zh-TW.html)）新增「新增 / 修改 AI 模型」章節，說明改碼 → 重新建構 → 驗證的完整流程。
