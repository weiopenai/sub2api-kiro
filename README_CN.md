...
### 2026-06-21
- 新增 **Claude Opus 4.8** 支持：`claude-opus-4-8` 与 `claude-opus-4-8-thinking` 已加入 Kiro 模型列表，并映射到上游模型 `claude-opus-4.8`。
- 部署手册（[deploy/usage-guide-zh-TW.html](deploy/usage-guide-zh-TW.html)）新增「新增 / 修改 AI 模型」章节，说明改码 → 重新构建 → 验证的完整流程。
