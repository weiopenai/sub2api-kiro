...
Sub2API Kiro Edition 是一个自部署 AI API 网关，用于把多个上游订阅账号的额度统一分发给用户和客户端。它保留了 Sub2API 原有的账号调度、分组、计费、管理后台、OpenAI/Anthropic 兼容接口，并加入了完整的 Kiro 平台支持。
...
当前 Go module path 仍保留为 `github.com/Wei-Shaw/sub2api`，目的是避免大规模 import path 改名带来的风险。公开仓库、Docker 镜像和文档由 `xiangking/sub2api` 独立维护。
